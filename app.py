...
def carregar_agendamentos():
    try:
        ...
        dados = response.data
        for ag in dados:
            # Converte string ISO para objeto datetime do Python
    ...
    with st.container():
        ...
        if botao_agendar:
            if not cliente:
                ...
                dt_completo = datetime.combine(
                    data_atendimento, hora_atendimento
                )
                lista_agendamentos = carregar_agendamentos()
                conflito = any(
                    ag["profissional"] == profissional
                    and ag["data_hora"] == dt_completo
                    for ag in lista_agendamentos
                )
                ...
                if not conflito:
                    ...
                    if len(tel_limpo) == 9:
                        ...
                        not tel_limpo.startswith("55") and len(tel_limpo) >= 10
                    ):
                        tel_limpo = "55" + tel_limpo
                    ...
                    if sucesso:
                        ...
                        st.success(
                            f"🎉 Horário reservado com sucesso para {cliente}!"
                        )
...
with aba2:
    ...
    data_consulta_sel = st.selectbox(
        "Selecione a data para consultar:",
        opcoes_datas_consulta,
        format_func=lambda x: x[1],
        key="select_data_consulta",
    )
    ...
    sub_bruno, sub_samuel, sub_geral = st.tabs(
        ["🧔 Bruno", "👨 Samuel", "📋 Visão Geral"]
    )
    ...
    def renderizar_lista(lista_filtrada):
        ...
        for ag in lista_filtrada:
            ...
            msg_encodada = urllib.parse.quote(
                f"Olá, {ag['cliente']}! Seu horário para {ag['servico']} está confirmado para o dia {data_str} às {hora_str} com o profissional {ag['profissional']}. 💈"
            )

            destino_whatsapp = ag.get("telefone", "")
            link_whatsapp = (
                f"https://wa.me/{destino_whatsapp}?text={msg_encodada}"
                if destino_whatsapp
                else f"https://wa.me/?text={msg_encodada}"
            )
    ...
    with sub_bruno:
        ag_bruno_data = [
            ag
            for ag in lista_agendamentos
            if ag["profissional"] == "Bruno"
            and ag["data_hora"].date() == data_consulta
        ]
        vagos_bruno = obter_horarios_vagos("Bruno", data_consulta)

        with st.expander(
            f"🟢 Ver {len(vagos_bruno)} horários vagos do Bruno para {data_consulta.strftime('%d/%m/%Y')}",
            expanded=False,
        ):
            if vagos_bruno:
                cols = st.columns(4)
                for idx, hr in enumerate(vagos_bruno):
                    cols[idx % 4].button(
                        f"⏰ {hr}", key=f"vago_bruno_{hr}", disabled=True
                    )
            else:
    ...
    with sub_samuel:
        ag_samuel_data = [
            ag
            for ag in lista_agendamentos
            if ag["profissional"] == "Samuel"
            and ag["data_hora"].date() == data_consulta
        ]
        vagos_samuel = obter_horarios_vagos("Samuel", data_consulta)

        with st.expander(
            f"🟢 Ver {len(vagos_samuel)} horários vagos do Samuel para {data_consulta.strftime('%d/%m/%Y')}",
            expanded=False,
        ):
            if vagos_samuel:
                cols = st.columns(4)
                for idx, hr in enumerate(vagos_samuel):
                    cols[idx % 4].button(
                        f"⏰ {hr}", key=f"vago_samuel_{hr}", disabled=True
                    )
            else:
                st.warning("Nenhum horário livre para este dia!")
    ...
    with sub_geral:
        ag_geral_data = [
            ag
            for ag in lista_agendamentos
            if ag["data_hora"].date() == data_consulta
        ]
...
with aba3:
    ...
    if not lista_agendamentos:
        ...
        canc_bruno, canc_samuel, canc_geral = st.tabs(
            ["🧔 Cancelar Bruno", "👨 Cancelar Samuel", "📋 Ver Todos"]
        )
    ...
    if senha == "admin123":
        ...
        if not lista_agendamentos:
            ...
            filtro_tempo = st.selectbox(
                "Período:", ["Todo o Período", "Este Mês", "Esta Semana"]
            )
            ...
            for ag in lista_agendamentos:
                ...
                if filtro_tempo == "Este Mês" and (
                    data_ag.year != hoje.year or data_ag.month != hoje.month
                ):
                    continue
                if filtro_tempo == "Esta Semana" and not (
            ...
            if agendamentos_filtrados:
                ...
                faturamento_total = sum(
                    PRECOS_SERVICOS.get(ag["servico"], 0)
                ...
                fat_bruno = sum(
                    PRECOS_SERVICOS.get(ag["servico"], 0) for ag in ag_bruno
                )
                fat_samuel = sum(
                    PRECOS_SERVICOS.get(ag["servico"], 0) for ag in ag_samuel
                )

                contagem_servicos = {
                    s: {"Bruno": 0, "Samuel": 0, "Total": 0}
                    for s in PRECOS_SERVICOS.keys()
                }
                for ag in agendamentos_filtrados:
                    s = ag["servico"]
                    p = ag["profissional"]
                ...
                def servico_mais_rentavel(lista_ag):
                    ...
                    for ag in lista_ag:
                        s = ag["servico"]
                        rendimento_por_servico[s] = rendimento_por_servico.get(
                            s, 0
                        ) + PRECOS_SERVICOS.get(s, 0)
                    return max(
                        rendimento_por_servico, key=rendimento_por_servico.get
                    )
                ...
                with col_m1:
                    st.markdown(
                        f'<div class="metric-card"><span style="color:var(--text-color);opacity:0.7;font-size:14px;font-weight:700;">Faturamento Total</span><br><span style="font-size:26px;font-weight:800;color:#23a55a;">R$ {faturamento_total:,.2f}</span></div>',
                        unsafe_allow_html=True,
                    )
                with col_m2:
                    st.markdown(
                        f'<div class="metric-card"><span style="color:var(--text-color);opacity:0.7;font-size:14px;font-weight:700;">Total de Atendimentos</span><br><span style="font-size:26px;font-weight:800;color:var(--text-color);">{total_atendimentos}</span></div>',
                        unsafe_allow_html=True,
                    )

                st.markdown(
                    "<h3 style='margin-top:25px;'>Desempenho por Barbeiro</h3>",
                    unsafe_allow_html=True,
                )
                ...
                st.markdown(
                    "<h3 style='margin-top:25px;'>Quantidade de Serviços</h3>",
                    unsafe_allow_html=True,
                )
                ...
                tabela_html = (
                    f"<div class='metric-card' style='padding: 15px; overflow-x: auto; text-align: left;'>"
